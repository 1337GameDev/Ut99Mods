class Key expands Actor;

#exec MESH IMPORT MESH=Key ANIVFILE=MODELS\Key_a.3d DATAFILE=MODELS\Key_d.3d X=0 Y=0 Z=0
#exec MESH ORIGIN MESH=Key X=0 Y=0 Z=0

#exec MESH SEQUENCE MESH=Key SEQ=All STARTFRAME=0 NUMFRAMES=30
//#exec MESH SEQUENCE MESH=Key SEQ=??? STARTFRAME=0 NUMFRAMES=30

#exec MESHMAP NEW MESHMAP=Key MESH=Key
#exec MESHMAP SCALE MESHMAP=Key X=0.1 Y=0.1 Z=0.2

#exec TEXTURE IMPORT NAME=Jtex1 FILE=texture1.pcx GROUP=Skins FLAGS=2
#exec TEXTURE IMPORT NAME=Jtex1 FILE=texture1.pcx GROUP=Skins PALETTE=Jtex1
#exec MESHMAP SETTEXTURE MESHMAP=Key NUM=1 TEXTURE=Jtex1

defaultproperties
{
    DrawType=DT_Mesh
    Mesh=Key
}
