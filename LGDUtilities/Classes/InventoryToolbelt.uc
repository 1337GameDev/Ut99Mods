...
static function InventoryToolbelt GetCurrentPlayerInventoryToolbeltHudInstance(Actor context, PlayerPawn pp){
    ...
    foreach context.AllActors(class'PlayerPawn', pp) {
       m = class'LGDUtilities.HUDMutator'.static.GetHUDMutatorFromPlayerPawnByClassName(pp, 'InventoryToolbelt');
    ...
    if(tb == None){
       tb = class'LGDUtilities.InventoryToolbelt'.static.SpawnAndRegister(context);
    }
...
static function InventoryToolbelt SpawnAndRegister(Actor context){
    ...
    hud = context.Spawn(class'LGDUtilities.InventoryToolbelt');
    hud.InventoryItemsInBelt = new class'LGDUtilities.LinkedList';
...
function bool AddInventoryToToolbelt(object inv){
    ...
    if(InventoryItemsInBelt != None){
        InventoryItemsInBelt = new class'LGDUtilities.LinkedList';
    }
    ...
    if(itemElement == None){//not a ToolbeltInvItem object
        ...
        if(invToAdd != None){//IS an Inventory object
            itemElement = class'LGDUtilities.ToolbeltInvItem'.static.GetInstanceForInvItem(invToAdd);
        }
    }
...
defaultproperties {
      ...
      ToolbeltSlotTexture=Texture'LGDUtilities.Background.ToolbeltSlotBackground'
