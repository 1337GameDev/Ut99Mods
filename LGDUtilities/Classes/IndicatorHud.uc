...
var float ScaleIndicatorMaxSizePercentOfScreenLargestDimension;//maximum % of largest screen dimension
var float ScaleIndicatorMinSizePercentOfScreenLargestDimension;//minimum % of largest screen dimension
var float ScaleIndicatorMaxSizePercentOfScreenSmallestDimension;//maximum % of largest screen dimension
var float ScaleIndicatorMinSizePercentOfScreenSmallestDimension;//minimum % of largest screen dimension
var float ScaleIndicatorMaxSizePercentOfScreenHorizontalDimension;//maximum % of horizontal screen dimension
var float ScaleIndicatorMinSizePercentOfScreenHorizontalDimension;//minimum % of horizontal screen dimension
var float ScaleIndicatorMaxSizePercentOfScreenVerticalDimension;//maximum % of vertical screen dimension
var float ScaleIndicatorMinSizePercentOfScreenVerticalDimension;//minimum % of vertical screen dimension
...
function PreBeginPlay() {
    ...
    markerMut = class'LGDUtilities.MutatorHelper'.static.GetGameMutatorByClass(self, class'LGDUtilities.DroppedInventoryMarkerMutator');
    if(markerMut == None){
        class'LGDUtilities.DroppedInventoryMarkerMutator'.static.RegisterMutator(self);
    }
}
...
simulated final function AddBasicTarget(Actor target, optional bool globalTarget, optional object SourceToLimit, optional bool replaceExistingTarget){
	...
	indicatorHudEl.IndicatorSettings = new class'LGDUtilities.IndicatorSettings';
...
final function bool IfTargetInList(Actor targetToFind, optional bool CheckGlobal, optional object SourceToLimit) {
	return (self.GetTargetElementFromList(targetToFind, CheckGlobal, SourceToLimit) != None);
}

final function ListElement GetTargetElementFromList(Actor targetToFind, optional bool CheckGlobal, optional object SourceToLimit) {
	local IndicatorHudGlobalTargets globalTargets;
	...
	while(!FoundTheElement  && (el != None)) {
		if(el.Value == targetToFind) {
			if(SourceToLimit != None) {
				ihLe = IndicatorHudTargetListElement(el);
				if(ihLe != None) {
					FoundTheElement = (ihLe.IndicatorSource == SourceToLimit);
				}//not a list element, so continue, as we are wanting to also match the source
			} else {
...
function int ResetAllTargetsForSource(object SourceToLimit) {
	local int countRemoved;
	local ListElement element, nextElement;
	local IndicatorHudTargetListElement ihElement;
...
//
// The main function for drawing a list of targets' indicators.
//=============================================================================
simulated final function DrawIndicatorLocations(Canvas C, PlayerPawn Player, LinkedList Targets) {
      ...
      if((Player != None) && (Targets != None) && (Targets.Count > 0)){
          ...
          While(element!=None && element.Value != None) {
              ...
              //conditionally set the position if we wnat the indicator above
              if(ShowIndicatorAboveTarget) {
			      class'LGDUtilities.HUDHelper'.static.getActorVerticalSizeOnHudFromColliderWithPoints(C, target, targetTopPos, targetBottomPos);
              ...
              if(ShowTargetDistanceLabels){
                  DistanceAsMeters = class'LGDUtilities.MathHelper'.static.UUtoMeters(CurrentTargetDistance);
              ...
              isOffScreen = class'LGDUtilities.HUDHelper'.static.IsOffScreen(C, targetScreenXPos, targetScreenYPos, offLeft_int, offRight_int, offTop_int, offBottom_int, 15);
              ...
              if(UseTriangleQuadrantsForOffScreen) {
                isInAnyTri = class'LGDUtilities.HUDHelper'.static.GetScreenTrianglesPointIsIn(C, targetScreenXPos, targetScreenYPos, topTri_int, bottomTri_int, leftTri_int, rightTri_int);
              ...
              if(targetIsBehind || isOffScreen) {
                  if(!ShowIndicator || (StaticIndicatorPercentOfMinScreenDimensionWhenOffScreen <= 0)) {
                      ...
                      finalIndicatorScale = class'LGDUtilities.HUDHelper'.static.getScaleForTextureFromMaxTextureDimension(IndicatorTextureToDisplay, StaticIndicatorPercentOfMinScreenDimensionWhenOffScreen * screenSmallestDimension);
                  }
              ...
              if(ShowIndicator) {
                  if(UseTextOnlyIndicators) {
                      ...
                      C.DrawColor = class'LGDUtilities.ColorHelper'.default.WhiteColor;
                      ...
                      if(ShowIndicatorAboveTarget) {
                          ...
                          class'LGDUtilities.HUDHelper'.static.DrawTextureAtXY(C, IndicatorTextureToDisplay, drawAtScreenX, drawAtScreenY, finalIndicatorScale, PlayerHUDScale, true);
                      }
                  }
              }
              ...
              if(ShowTargetDistanceLabels){
                  C.Style = ERenderStyle.STY_Normal;
                  C.DrawColor = class'LGDUtilities.ColorHelper'.static.GetWhiteColor();
...
static function IndicatorHud SpawnAndRegister(Actor context){
    ...
    hudMut = class'LGDUtilities.HUDMutator'.static.GetHUDMutatorFromAnyPlayerPawnByClass(context, class'LGDUtilities.IndicatorHud');

    if(hudMut == None){
        hud = context.Spawn(class'LGDUtilities.IndicatorHud');
        hud.PlayerIndicatorTargets = new class'LGDUtilities.LinkedList';
        hud.GlobalIndicatorTargets = class'LGDUtilities.IndicatorHudGlobalTargets'.static.GetRef(context);
...
static function IndicatorTextureVariations GetTexturesForBuiltInOption(byte wantedBuiltinTex, optional byte offScreenTexDesired){//HUDIndicator_Texture_BuiltIn - wantedBuiltinTex
    ...
    texVariation = new class'LGDUtilities.IndicatorTextureVariations';

    if(offScreenTexDesired != HUDIndicator_Texture_BuiltIn.Empty){
        offScreenTexVariation = class'LGDUtilities.IndicatorHud'.static.GetTexturesForBuiltInOption(offScreenTexDesired);
    }
...
defaultproperties {
      ...
      IndicatorTexture=Texture'LGDUtilities.HudElement_Ring_B'
      BehindViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
      UseTriangleQuadrantsForOffScreen=True
      OffTopLeftViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
      OffLeftViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
      OffBottomLeftViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
      OffBottomViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
      OffBottomRightViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
      OffRightViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
      OffTopRightViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
      OffTopViewTexture=Texture'LGDUtilities.HudElement_Ring_B'
