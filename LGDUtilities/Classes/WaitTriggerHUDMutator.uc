		...
		if(TriggerActor.CanActivateTrigger() || (!TriggerActor.CanActivateTrigger() && TriggerActor.ShowMessagesAfterActivated)){
			if(TriggerActor.CanActivateTrigger() ){
				...
				if(TriggerActor.ShowWaitMessage){
					...
					class'LGDUtilities.HUDHelper'.static.DrawTextAtXY(C, self, TriggerActor.WaitMessage, WaitMessageXPos, WaitMessageYPos, true);
				}
				...
				if(TriggerActor.ShowTimeWaitedAt){
					...
					class'LGDUtilities.HUDHelper'.static.DrawTextAtXY(C, self, TimeWaitedAtMessage, WaitMessageXPos, WaitMessageYPos+WaitMessageSizeY+10, true);
				}
				...
				if(TriggerActor.ShowRetriggerCooldownMessage){
					...
					C.Style = ERenderStyle.STY_Normal;
					class'LGDUtilities.HUDHelper'.static.DrawTextAtXY(C, self, TriggerActor.RetriggerCooldownMessage, WaitMessageXPos, WaitMessageYPos, true);
				}
...
static function WaitTriggerHUDMutator GetCurrentPlayerWaitTriggerHudInstance(Actor context, PlayerPawn pp){
    ...
    foreach context.AllActors(class'PlayerPawn', pp) {
       m = class'LGDUtilities.HUDMutator'.static.GetHUDMutatorFromPlayerPawnByClassName(pp, 'WaitTriggerHUDMutator');
       if(m == None){
           continue;
       } else {
...
static function WaitTriggerHUDMutator SpawnAndRegister(Actor context){
    ...
    hud = context.Spawn(class'LGDUtilities.WaitTriggerHUDMutator');
