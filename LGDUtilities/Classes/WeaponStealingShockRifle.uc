        ...
        if((HitPawn != None) && (HitPawn.Weapon != None)){
            ...
            if(WeaponEligibleToBeStolen(HitPawn.Weapon)) {
                ...
                if((redeemer != None) && redeemer.bGuiding && redeemer.IsInState('Guiding')){
                     ...
                     redeemer.Fire(0);
                } else if(!((class'LGDUtilities.InventoryHelper'.static.GetItemCountInInventory(HitPawn, false) <= 1) && !AllowStealingLastWeapon)){
                    HasStolenWeapon = class'LGDUtilities.WeaponHelper'.static.StealWeapon(PlayerOwner, HitPawn, PreviouslyHeldWeapon, WeaponStolen);
                    
					if(HasStolenWeapon && (WeaponStolen != None)){
                        if(PlayerOwner.IsA('PlayerPawn')){
                            PlayerOwner.ReceiveLocalizedMessage(class'LGDUtilities.StealWeaponMessage', 0, None, None, WeaponStolen);
                        }

                        if(HitPawn.IsA('PlayerPawn')){
                            HitPawn.ReceiveLocalizedMessage(class'LGDUtilities.WeaponStolenMessage', 0, None, None, WeaponStolen);
                        }
                    }
                }
...
function SpawnEffect(vector HitLocation, vector SmokeLocation) {
	...
	Smoke = Spawn(class'LGDUtilities.StealShockBeam',,,SmokeLocation,SmokeRotation);
...
function bool WeaponEligibleToBeStolen(Weapon wep){
    ...
    if(wep != None){
        ...
        //Maybe we can key off of wep.bMeleeWeapon to limit this further? Might not be needed
        ...
        if(wep.IsA('ImpactHammer') && !AllowStealingImpactHammer){
            ...
            CanBeStolen = false;
        } else if(wep.IsA('WeaponEventListenerWeapon') || wep.IsA('ItemSpawnerWeapon')){
            CanBeStolen = false;
        } else {
