...
static function ToolbeltInvItem GetInstanceForInvItem(Inventory inv){
    ...
    if(inv != None){
        toolbeltItem = new class'ToolbeltInvItem';
        toolbeltItem.Value = inv;
        if(inv.Icon == None){
            toolbeltItem.ToolbeltIcon = class'InventoryToolbelt'.default.DefaultIcon;
        } else {
            toolbeltItem.ToolbeltIcon = inv.Icon;
        }

        toolbeltItem.TexScaleToBe64Pixels = class'HudHelper'.static.getScaleForTextureToGetDesiredWidth(toolbeltItem.ToolbeltIcon, 64.0);
    }
