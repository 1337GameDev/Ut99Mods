...
static function LinkedList GetAllWeaponClasses(Actor context, optional bool OnlyIncludeBaseWeapons, optional bool ExcludeChaosUTWeapons, optional bool bLogToGameLogfile) {
    ...
    FoundClasses = class'ServerHelper'.static.GetClassesLoadedFromIntFiles(context, "Engine.Weapon");
    //get UT99 weapons, and any with meta of Botpack.TournamentWeapon
    FoundClasses.Concat(class'ServerHelper'.static.GetClassesLoadedFromIntFiles(context, "Botpack.TournamentWeapon") );

    if(!OnlyIncludeBaseWeapons) {
        //get Wormbo EnhanceWeapon weapons, and any with meta of EnhancedItems.EnhancedWeapon
        FoundClasses.Concat(class'ServerHelper'.static.GetClassesLoadedFromIntFiles(context, "EnhancedItems.EnhancedWeapon") );
    } else {
        ...
        while(el != None) {
            ...
            if((el.Value != None) && ClassIsChildOf(el.Value.class, class'ClassObj')) {
                classObjVal = ClassObj(el.Value);
                classPackage = class'StringHelper'.static.GetPackageNameFromQualifiedClass(classObjVal.ClassQualifiedName);
    ...
    while(el != None) {
		...
		if((el.Value != None) && ClassIsChildOf(el.Value.class, class'ClassObj')) {
			classObjVal = ClassObj(el.Value);
            className = class'StringHelper'.static.RemovePackageNameFromQualifiedClass(classObjVal.ClassQualifiedName);
...
static function LinkedList GetClassesLoadedFromIntFiles(Actor context, string IntMetaClassToCompareTo, optional bool LoadClasses, optional int MaxClassIntNum, optional bool bLogToGameLogfile){
    ...
    FoundClasses = new class'LinkedList';
	...
	while((NextClassNameToLoad != "") && (ClassIntNum < MaxClassIntNum)) {
        ...
        class'ClassObj'.static.PushClassOntoLinkedList(FoundClasses, LoadedClass, NextClassNameToLoad);
