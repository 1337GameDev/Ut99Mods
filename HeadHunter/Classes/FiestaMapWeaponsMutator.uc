...
MutatorBeforeThis = class'MutatorHelper'.static.GetMutatorBeforeMutatorInChain(Self);
...
function PostBeginPlay() {
    Log("FiestaMapWeaponsMutator - HandleRestartGame");
		...
		while(le != None) {
		    if(Ammo(le.Value) != None) {
				...
				if(w != None) {
					...
					if(wepClass != None) {
						Log("FiestaMapWeaponsMutator - ReplaceMapWeapons - Replacing: "$w.Name$" - new wepClass: "$wepClass.Name);
					} else {
						Log("FiestaMapWeaponsMutator - ReplaceMapWeapons - wepClass:None");
					}
					...
					if(newWep != None) {
						...
						Log("FiestaMapWeaponsMutator - ReplaceMapWeapons - newWep was NONE");
					}
...
function InitRandomlyChosenWeaponsList() {
	...
	if(!HasInitArrays) {
	    weps = class'ServerHelper'.static.GetAllWeaponClasses(self);
...
function MoveThisMutatorToFirstInChain() {
    ...
    if(MutatorAfterThis != None) {
        Log("FiestaMapWeaponsMutator - MoveThisMutatorToFirstInChain - MutatorAfterThis:"$MutatorAfterThis.Name);
    } else {
        Log("FiestaMapWeaponsMutator - MoveThisMutatorToFirstInChain - MutatorAfterThis:None");
    }

    Log("FiestaMapWeaponsMutator - MoveThisMutatorToFirstInChain - MutatorBeforeThis:"$MutatorBeforeThis.Name);
    Log("FiestaMapWeaponsMutator - MoveThisMutatorToFirstInChain - GameBaseMutator:"$GameBaseMutator.Name);
