...
static simulated function DrawLine3D(Canvas C, vector P1, vector P2, float R, float G, float B){
	...
	a1 = class'HUDHelper'.static.getXY(C, P1, x1, y1);
	a2 = class'HUDHelper'.static.getXY(C, P2, x2, y2);
	...
	if(a1 <= 0){
		...
		P1 -= Normal(P1);
		P1 += P2;
		class'HUDHelper'.static.getXY(C, P1, x1, y1);
	} else if(a2 <= 0){
		P2 -= P1;
		P2 *= a1/(a1 - a2);
		P2 -= Normal(P2);
		P2 += P1;
		class'HUDHelper'.static.getXY(C, P2, x2, y2);
	}
	...
	class'HUDHelper'.static.DrawLine(C, x1, y1, x2, y2);
}
...
static simulated function DrawCircleAtPosWithWidth(Actor context, Canvas canvas, int screenX, int screenY, float wantedWidth, float playerHudScale) {
      ...
      resultingScale = class'HUDHelper'.static.getScaleForTextureFromMaxDimension(tex, wantedWidth);
      canvas.Style = context.ERenderStyle.STY_Translucent;
      class'HUDHelper'.static.DrawTextureAtXY(canvas, tex, screenX, screenY, resultingScale, playerHudScale, true, true);
}
...
static simulated function DrawTextAtXY(Canvas canvas, Actor context, string text, int screenX, int screenY, optional bool centerText) {
      ...
      canvas.DrawColor = class'ColorHelper'.static.GetWhiteColor();;
...
//given a texture (and based on it's current height), and a desired height, this returns a scale to use for the texture to get the desired height
static simulated function float getScaleForTextureFromMaxDimension(Texture tex, float wantedMaxDimensionSize) {
