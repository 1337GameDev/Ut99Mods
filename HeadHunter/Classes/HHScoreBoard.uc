...
// HHScoreBoard
// ============================================================
class HHScoreBoard expands TournamentScoreBoard;

var color LightGreenColor, DarkGreenColor;
var Texture SkullTex;
...
function DrawVictoryConditions(Canvas Canvas) {
	...
	local float XL, YL, TexScale, XPosToDrawGameName, XPosForSkull;
    ...
    Canvas.DrawColor = GoldColor;
    Canvas.DrawIcon(SkullTex, TexScale);

    Canvas.SetPos(XPosToDrawGameName, Canvas.CurY);
    Canvas.DrawColor = RedColor;
	Canvas.DrawText(TGRI.GameName);

    XPosForSkull = (Canvas.SizeX / 2.0) + (XL / 2.0);
	Canvas.SetPos(XPosForSkull, Canvas.CurY - (YL) - 64);
	Canvas.DrawColor = GoldColor;
    Canvas.DrawIcon(SkullTex, TexScale);
