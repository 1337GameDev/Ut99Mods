...
simulated final function DrawLookIndicatorLocation(Canvas C, PlayerPawn Player) {
    ...
    if((Player != None) && (TriggerActor != None)){
        ...
        if(ShowIndicator) {
            ...
            class'HUDHelper'.static.DrawTextureAtXY_OutputEdgeCordinates(C, IndicatorTextureToDisplay, drawAtScreenX, drawAtScreenY, finalIndicatorScale, PlayerHUDScale, True, False, Indicator_TopL, Indicator_TopR, Indicator_BottomL, Indicator_BottomR);
            ...
            class'HUDHelper'.static.DrawTextAtXY(C, self, "X", Indicator_CenterX, Indicator_CenterY, true);

            //show the "target" to look at in the screen center
            if(TriggerActor.ShowTargetLookCircle){
                class'HUDHelper'.static.DrawCircleMidScreenWithWidth(self, C, TriggerActor.TargetLookCircleWidth, PlayerHUDScale);
            }

            if(VSize(screenMiddlePos - Indicator_Center) > ((TriggerActor.TargetLookCircleWidth*PlayerHUDScale))) {//diff between center of screen and center of indictaor, outside circle dimensions
            ...
            if(TriggerActor.CanActivateTrigger() || (!TriggerActor.CanActivateTrigger() && TriggerActor.ShowMessagesAfterActivated)){
                if(TriggerActor.CanActivateTrigger()){
                    ...
                    if(TriggerActor.ShowLookMessage){
					    ...
					    class'HUDHelper'.static.DrawTextAtXY(C, self, TriggerActor.LookMessage, LookMessageXPos, LookMessageYPos, true);
				    } else {
                    ...
                    if(TriggerActor.ShowTimeLookedAt){
						...
						class'HUDHelper'.static.DrawTextAtXY(C, self, TimeLookedAtMessage, LookMessageXPos, LookMessageYPos+LookMessageSizeY+10, true);
					}
                    ...
                    if(TriggerActor.ShowRetriggerCooldownMessage){
					    ...
					    LookMessageYPos = Indicator_BottomL.Y+LookMessageSizeY;
					    C.Style = ERenderStyle.STY_Normal;
					    class'HUDHelper'.static.DrawTextAtXY(C, self, TriggerActor.RetriggerCooldownMessage, LookMessageXPos, LookMessageYPos, true);
                    } else {
					    LookMessageYPos = Indicator_BottomL.Y;
                    }
...
static function LookTriggerHUDMutator GetCurrentPlayerLookTriggerHudInstance(Actor context, PlayerPawn pp){
    ...
    foreach context.AllActors(class'PlayerPawn', pp) {
       m = class'HUDMutator'.static.GetHUDMutatorFromPlayerPawnByClassName(pp, 'LookTriggerHUDMutator');
       if(m == None){
           continue;
       } else {
...
static function LookTriggerHUDMutator SpawnAndRegister(Actor context){
    ...
    hud = context.Spawn(class'LookTriggerHUDMutator');
